...
def draw_foot_preview_cv2(skeleton: dict, params: FootParams, size: tuple[int, int] = (860, 620)) -> np.ndarray:
    ...
    if params.preview_mode == "top":
        _draw_top_view(canvas, skeleton, params, (0, 0, width, height), "上面図")
    elif params.preview_mode == "side":
        _draw_side_view(canvas, skeleton, params, (0, 0, width, height), "側面図")
    else:
        top_h = int(height * 0.62)
        _draw_top_view(canvas, skeleton, params, (0, 0, width, top_h), "上面図")
        cv2.line(canvas, (24, top_h), (width - 24, top_h), (210, 214, 220), 1)
        _draw_side_view(canvas, skeleton, params, (0, top_h, width, height - top_h), "側面図")
    return canvas
...
def _draw_top_view(canvas: np.ndarray, skeleton: dict, params: FootParams, rect: tuple[int, int, int, int], title: str) -> None:
    ...
    for toe in skeleton["toes"]:
        ...
        for a, b in zip(chain, chain[1:]):
            _line(canvas, p(a), p(b), (70, 92, 112), 3)
    ...
    for toe in skeleton["toes"]:
        for key in ["base", "mid", "distal"]:
            _joint(canvas, p(toe[key]), 4, (35, 132, 105))
        _joint(canvas, p(toe["tip"]), 6, (200, 88, 64))
...
def _draw_side_view(canvas: np.ndarray, skeleton: dict, params: FootParams, rect: tuple[int, int, int, int], title: str) -> None:
    ...
    pts = skeleton["points"]
    sole = [
        np.array([0.0, -params.foot_length * 0.42, 0.0]),
        np.array([0.0, -params.foot_length * 0.15, params.arch_height * 0.28]),
        np.array([0.0, params.foot_length * 0.35, 0.0]),
        np.array([0.0, params.foot_length * 0.68, 0.0]),
    ]
    ...
    for key in ["ankle", "heel", "instep", "arch", "big_ball"]:
        _joint(canvas, p(pts[key]), 5, (48, 114, 164))
    label = f"Instep:{params.instep_height:.0f}  Arch:{params.arch_height:.0f}  Lift:{params.toe_lift:.0f}  Curl:{params.toe_curl:.0f}"
    cv2.putText(canvas, label, (x0 + 18, y0 + h - 16), cv2.FONT_HERSHEY_SIMPLEX, 0.48, (78, 88, 102), 1, cv2.LINE_AA)
...
def _side_bounds(skeleton: dict, params: FootParams) -> tuple[float, float, float, float]:
    values = list(skeleton["points"].values())
    for toe in skeleton["toes"]:
        values.extend([toe["base"], toe["mid"], toe["distal"], toe["tip"]])
    arr = np.array(values)
